/**
 * JavaScript cho trang Hệ Thống Ánh Sáng
 * Namespace: lightingSystemPage
...
(function ($) {
  ...
  var lightingSystemPage = {
    // Initialize
    init: function () {
      // Chỉ chạy trên trang hệ thống ánh sáng
      if (!$(".lighting-system-page").length) {
        return;
      }
    ...
    initSliders: function () {
      var sliders = $(".lighting-system-page .products-carousel");
      if (!sliders.length) {
        return;
      }
      ...
      sliders.each(function () {
        ...
        prevBtn.off("click.lightSlider").on("click.lightSlider", function () {
          if (currentIndex > 0) {
            currentIndex--;
            moveSlider();
          }
        ...
        nextBtn.off("click.lightSlider").on("click.lightSlider", function () {
        ...
        track
          .off("touchstart.lightDrag mousedown.lightDrag")
          .on("touchstart.lightDrag mousedown.lightDrag", function (e) {
        ...
        track
          .off("touchmove.lightDrag mousemove.lightDrag")
          .on("touchmove.lightDrag mousemove.lightDrag", function (e) {
        ...
        track
          .off("touchend.lightDrag mouseup.lightDrag mouseleave.lightDrag")
          .on(
            "touchend.lightDrag mouseup.lightDrag mouseleave.lightDrag",
            function () {
              if (!isDragging) return;
              isDragging = false;
              track
        ...
        track
          .off("wheel.lightingCategory")
          .on("wheel.lightingCategory", function (event) {
        ...
        var resizeTimer;
        $(window)
          .off("resize.lightSlider" + slider.data("carousel"))
          .on("resize.lightSlider" + slider.data("carousel"), function () {
  ...
  $(document).ready(function () {
    lightingSystemPage.init();
  });

  // Reinitialize on AJAX content load (if needed)
  $(document).on("lightingSystemPageReinit", function () {
    lightingSystemPage.init();
  });
})(jQuery);
